...
C0, C5, user-data encryption, C6, and C7 constraint counts did not change. All isolated proof sizes
stayed at `14.31 KB`.
...
`3.0.0-nightly.20260102`. The current run uses Nargo `1.0.0-beta.26` and Barretenberg `5.1.0`. The
size and timing deltas below are therefore directional, not strict toolchain-controlled regressions.

### C2 Topology Note

The `C2a` and `C2b` rows below are still generated from the legacy monolithic benchmark entrypoints,
`dkg/sk_share_computation` and `dkg/e_sm_share_computation`. They do not represent the total cost of
the current chunked C2 path and must not be read as a per-track regression.
...
_P2 **tracked_job_wall** sum (ZkDkgAggregation + ZkPkAggregation, parallelizable): **18.52 s** — not
...
Sum of tracked job wall time: **7084.91 s** — **not** end-to-end latency (jobs run in parallel up to
