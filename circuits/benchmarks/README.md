...
of the committee's `(N, T)` and the preset's threshold `QIS`, so any pair compiles and proves
correctly.
...
(`insecure` for `--mode insecure`, `secure-8192` for `--mode secure`). For **insecure** runs it
...
that `.sol` diff (committed verifiers stay pinned to `insecure` for production deploy); gas
