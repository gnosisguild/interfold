...
#
# Usage: ./check_circuit_preset_artifacts.sh <insecure-512|secure-8192|secure-16384> [--committee minimum|micro|small]

set -e

PRESET="${1:-}"
if [ -z "$PRESET" ]; then
    echo "Usage: $0 <insecure-512|secure-8192|secure-16384> [--committee minimum|micro|small]" >&2
    exit 1
fi
if [ "$PRESET" != "insecure-512" ] && [ "$PRESET" != "secure-8192" ] && [ "$PRESET" != "secure-16384" ]; then
    echo "Error: preset must be insecure-512, secure-8192, or secure-16384" >&2
    exit 1
fi
