...
//! and (for input) authorized ciphertexts and secret key. Input values are normalized for the ZKP
...
use crate::Individual_key::share_decryption::ShareDecryptionCircuit;
use crate::Individual_key::share_decryption::ShareDecryptionCircuitData;
...
/// Global configs for the share-decryption circuit: degree, number of moduli, authorized parties, bits, and bounds.
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Inputs {
    /// Expected message commitments from share-encryption (CIRCUIT 3) for H authorized parties: [party_idx][mod_idx].
...
impl Computation for Configs {
    ...
    fn compute(preset: Self::Preset, data: &Self::Data) -> Result<Self, CircuitsErrors> {
        ...
        let a = data.authorized_ciphertexts.len();
...
impl Computation for Inputs {
    ...
    fn compute(preset: Self::Preset, data: &Self::Data) -> Result<Self, Self::Error> {
        ...
        let has_own_slot = data.authorized_ciphertexts.iter().any(|s| s.is_none());
        ...
        // Iterate H slots in ascending authorized-party order: external slots BFV-decrypt and
        // commit; the own slot uses the supplied plaintext directly.
        for slot in data.authorized_ciphertexts.iter() {
            ...
            match slot {
                Some(party_cts) => {
                    if party_cts.len() < threshold_l {
                        return Err(CircuitsErrors::Other(format!(
                            "authorized_ciphertexts party has {} ciphertexts but threshold_l is {}; \
                    ...
                    for (mod_idx, ct) in party_cts.iter().enumerate().take(threshold_l) {
                        let decrypted_pt = data.secret_key.try_decrypt(ct).map_err(|e| {
                            CircuitsErrors::Other(format!(
                                "failed to decrypt authorized ciphertext at modulus {}: {:?}",
                                mod_idx, e
                            ))
                        })?;
...
mod tests {
    ...
    fn test_constants_json_roundtrip() {
        ...
        assert_eq!(decoded.a, constants.a);
    ...
    fn test_input_decryption_consistency() {
        ...
        // Inputs should have one row per authorized party
        assert_eq!(
            inputs.expected_commitments.len(),
            sample.authorized_ciphertexts.len()
        );
        assert_eq!(
    ...
    fn test_commitment_ordering_consistency() {
        ...
        for (party_idx, slot) in sample.authorized_ciphertexts.iter().enumerate() {
            for mod_idx in 0..threshold_l {
