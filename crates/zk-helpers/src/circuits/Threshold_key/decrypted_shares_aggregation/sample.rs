...
use crate::Threshold_key::decrypted_shares_aggregation::computation::Configs;
use crate::CircuitsErrors;
use crate::{
    Threshold_key::decrypted_shares_aggregation::DecryptedSharesAggregationCircuitData,
...
impl DecryptedSharesAggregationCircuitData {
    ...
    pub fn generate_sample(
        ...
        let authorized_parties = threshold + 1;
        let mut d_share_polys: Vec<Poly<PowerBasis>> = Vec::with_capacity(authorized_parties);

        for party in parties.iter().take(authorized_parties) {
            let share_manager = ShareManager::new(num_parties, threshold, threshold_params.clone())
                .map_err(|e| {
                    CircuitsErrors::Sample(format!("Failed to create ShareManager: {:?}", e))
                })?;
        ...
        let reconstructing_parties: Vec<usize> = (1..=authorized_parties).collect();
...
mod tests {
    use crate::{
        ...
        Threshold_key::decrypted_shares_aggregation::{DecryptedSharesAggregationCircuitData, Inputs},
    ...
    fn test_generate_sample() {
        ...
        let configs =
            crate::Threshold_key::decrypted_shares_aggregation::computation::Configs::compute(
                preset,
                &(),
            )
            .unwrap();
