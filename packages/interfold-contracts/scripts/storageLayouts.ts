...
export function assertInterfoldPricingSlots(layout: StorageLayout): string[] {
  const errors: string[] = [];
  const pricing = layout.storage.find(
    (entry) => entry.label === "_pricingConfig",
  );
  if (!pricing || pricing.slot !== "24" || pricing.offset !== 0) {
    return ["Interfold: _pricingConfig must begin at slot 24+0."];
  }

  const expected = [
    ["keyGenFixedPerNode", "0", 0],
    ["keyGenPerEncryptionProof", "1", 0],
    ["coordinationPerPair", "2", 0],
    ["availabilityPerNodePerSec", "3", 0],
    ["decryptionPerNode", "4", 0],
    ["publicationBase", "5", 0],
    ["verificationPerProof", "6", 0],
    ["protocolTreasury", "7", 0],
    ["marginBps", "7", 20],
    ["protocolShareBps", "7", 22],
    ["dkgUtilizationBps", "7", 24],
    ["computeUtilizationBps", "7", 26],
    ["decryptUtilizationBps", "7", 28],
    ["minCommitteeSize", "8", 0],
    ["minThreshold", "8", 4],
  ] as const;
  const members = layout.types[pricing.type]?.members ?? [];
  for (const [label, slot, offset] of expected) {
    const member = members.find((candidate) => candidate.label === label);
    if (!member || member.slot !== slot || member.offset !== offset) {
      errors.push(
        `Interfold: PricingConfig.${label} must remain at absolute slot ` +
          `${24n + BigInt(slot)}+${offset}.`,
      );
    }
  }
  const next = layout.storage.find(
    (entry) => entry.label === "_feeTokenAllowed",
  );
  if (!next || next.slot !== "33" || next.offset !== 0) {
    errors.push(
