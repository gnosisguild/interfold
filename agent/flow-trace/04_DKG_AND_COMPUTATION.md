...
`CRISPProgram.inputLeaf` therefore binds three values:

```
leaf = sha256(sha256(encryptedVote) || encryptedVoteCommitment || slotAddress) mod SNARK_SCALAR_FIELD
```
...
vector, and `crates/compute-provider/tests/onchain_root_agreement.rs` asserts Rust reproduces a root
a real contract produced, from a fixture generated by `tests/input-tree-e2e.test.ts`. A one-byte
divergence would make every root mismatch and nothing else would detect it.
...
For each slot the Secure Process computes over the **most recent entry whose bytes reproduce its
commitment**. Entries that do not reproduce it, or that do not deserialize, keep their leaf — removing
one would change the root — but are skipped, so selection falls back to that slot's last good entry.
...
- an append with bad bytes cannot erase a counted vote;
- an honest re-vote still replaces the earlier ballot, because it is the most recent usable entry;
- a mask preserves the tally, since the circuit forces its plaintext to zero
  (`check_coefficient_zero`) and proves `sum = prev + zero`.
...
`InputPublished` carries the slot and the commitment alongside the bytes. Both were already public —
the slot is a plaintext `publishInput` argument and `getSlotIndex` exposes it — so emitting them
leaks nothing and saves every consumer from parsing transaction calldata.
