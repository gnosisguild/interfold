...
- Ticket and license tokens, expected decimals, `ticketPrice`, and `licenseRequiredBond` change as
  ...
  bound to the exact BondingRegistry and asset-configuration version. The ticket token must
  authorize the BondingRegistry before operator collateral or status can change. — `flow-trace/02`,
  `05`; INDEX concern #23
