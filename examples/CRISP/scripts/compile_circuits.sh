...
# The generated verifiers are preset-specific: they are written from the compiled circuit's
# verification key, so a secure-8192 verifier cannot verify an insecure-512 proof. Read the preset
# out of the config the compile just used rather than taking it as an argument, so the output
# directory can never disagree with what was built.
MOD_NR="${INTERFOLD_CIRCUITS}/lib/src/configs/default/mod.nr"
PRESET="$(sed -n 's|^// Auto-generated by build-circuits.ts for preset: \(.*\)$|\1|p' "$MOD_NR")"
if [ -z "$PRESET" ]; then
    echo "Error: could not read the active preset from $MOD_NR"
    exit 1
fi
VERIFIER_DIR="packages/crisp-contracts/contracts/verifiers/${PRESET}"
mkdir -p "$VERIFIER_DIR"
echo "Generating verifiers for preset: ${PRESET} -> ${VERIFIER_DIR}"
