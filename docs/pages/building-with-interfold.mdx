...
2. Contract validates the parameters, quotes the service and flat randomness fees, and stores an
   `E3` record with a seed for the computation. The registry requests VRF and derives a separate,
   request-bound committee seed after a verified response.
