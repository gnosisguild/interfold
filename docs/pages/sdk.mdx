...
| ------------------------ | ------------------- | ------ | -------------- | -------- | ----------------------------- |
| `INSECURE_THRESHOLD_512` | `0`                 | 512    | 2              | ❌ None  | Local development and Sepolia |
| `SECURE_THRESHOLD_8192`  | `1`                 | 8192   | 3              | ✅ Full  | Production deployments        |
...
**Use `INSECURE_THRESHOLD_512` on Sepolia.** The Sepolia ciphernodes run the insecure preset, and
the circuit artifacts that ship in `@interfold/sdk` are compiled for it. The secure preset adds no
real security on a testnet that runs mock verifiers; it only makes proof generation slower and
causes a parameter mismatch. Earlier revisions of this page recommended `SECURE_THRESHOLD_8192` for
Sepolia — that recommendation was wrong.
...
const hash = await sdk.requestE3({
  ...
  e3Program: '0x...',
  paramSet: 0, // ParamSet.Insecure512 — use 1 (Secure8192) only on a secure network
...
from `CommitteeRandomnessRequested` to watch and correlate the provider response:
...
`getHistoricalRandomnessProviderEvents` returns fulfillment logs that predate the watcher. A
fulfillment means that the provider stored a response. Query the Registry to determine whether the
response was timely and usable.
...
- **Publish ciphertext output**:
  `sdk.publishCiphertextOutput(e3Id, ciphertextOutput, ciphertextCommitment, proof, gasLimit?)`
  submits the computed ciphertext output for an E3.
