...
initializes a Merkle tree to store votes efficiently. As votes are submitted, `publishInput()`
processes each encrypted vote by verifying its Noir zero-knowledge proof, ensuring votes are
correctly encrypted and voters are eligible. The round's census decides which verifier that is:
`CRISPVerifier` for a census posted as a Merkle root, and `CRISPOnchainVerifier` for one read from
token balances on chain. Votes are stored in the Merkle tree with slot addresses mapping to vote
indices, allowing updates while maintaining uniqueness.
