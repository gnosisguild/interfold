...
Install **nargo** (or rely on CI / Docker). The installer only places `noirup` on your `PATH`; run
`noirup` afterwards to get the pinned toolchain:
...
**bb** does not come from `noirup`. Use the Interfold CLI, which installs the Barretenberg version
pinned in `crates/zk-prover/versions.json` (`required_bb_version`) under `~/.interfold/noir` so it
matches the prover:
...
That copy is managed by the CLI rather than added to your `PATH`, so use `interfold noir status` to
check it instead of `bb --version`.
