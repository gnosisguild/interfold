...
using **sortition**, so membership follows publicly checkable ticket scoring rather than a central
picker. There is **no trusted dealer** handing out key material: every ciphernode contributes its
own share of the work and takes part in **PVDKG** (publicly verifiable DKG), producing proofs
alongside messages so that deviating parties can be identified and excluded without exposing other
nodes’ secrets.
...
- **P2 — Public key aggregation** is deliberately short: someone acting as aggregator sums the
  ...
  [**C5**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/pk_aggregation)
...
- **P3 — User encryption** is where data providers enter. They encrypt under the aggregated
  ...
  [`user_data_encryption_ct0`](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/user_data_encryption_ct0),
  [`user_data_encryption_ct1`](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/user_data_encryption_ct1),
  and
  [`user_data_encryption`](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/user_data_encryption)
...
- **P4 — Threshold decryption** closes the loop after homomorphic evaluation. Ciphernodes in **A**
  ...
  [**C6**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/share_decryption)
  and
  [**C7**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/decrypted_shares_aggregation)
...
[`circuits/bin/config`](https://github.com/theinterfold/interfold/tree/main/circuits/bin/config))
...
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| **Config**               | [`config`](https://github.com/theinterfold/interfold/tree/main/circuits/bin/config)                                                                                                                                                                                                                                                                                                                                                                                                                | One-time consistency of presets before any E3.                                                                        |
| **P1 — DKG**             | [**C0**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/pk), [**C1**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/pk_generation), [**C2**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/sk_share_computation_chunk), [**C3**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/share_encryption), [**C4**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/share_decryption) | Individual pk; TrBFV contribution; Shamir + parity (chunked recursive **C2**); encrypt shares; decrypt and aggregate. |
| **P2 — Aggregation**     | [**C5**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/pk_aggregation)                                                                                                                                                                                                                                                                                                                                                                                                | Sum honest public key shares into the threshold pk.                                                                   |
| **P3 — User encryption** | [`user_data_encryption_*`](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/user_data_encryption_ct0)                                                                                                                                                                                                                                                                                                                                                                    | Valid BFV encryption under the aggregated key.                                                                        |
| **P4 — Decryption**      | [**C6**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/share_decryption), [**C7**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/decrypted_shares_aggregation)                                                                                                                                                                                                                                                                           | Partial decryptions; Lagrange combination, CRT lift, decode to plaintext.                                             |
...
[**C1**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/pk_generation)
living under
[`circuits/bin/threshold`](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold)
and the rest of the DKG chain under
[`circuits/bin/dkg`](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg)).
...
[`circuits/lib/src/math/safe.nr`](https://github.com/theinterfold/interfold/blob/main/circuits/lib/src/math/safe.nr)),
...
[**C5**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/pk_aggregation),
and how
[**C4**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/share_decryption)
hands off to
[**C6**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/share_decryption),
without ever dumping raw polynomials into calldata. Domain labels (`DS_*` in
[`commitments.nr`](https://github.com/theinterfold/interfold/blob/main/circuits/lib/src/math/commitments.nr))
...
[**C2**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/sk_share_computation_chunk)'s
expected commitments match
[**C1**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/pk_generation)'s
...
1. **DKG recursive proof**: the aggregator recursively verifies every node's C0–C4 fold chain
   ...
   [`BfvPkVerifier`](https://github.com/theinterfold/interfold/blob/main/packages/interfold-contracts/contracts/verifiers/bfv/BfvPkVerifier.sol)
   at `publishCommittee`.
2. **Decryption recursive proof**: the aggregator recursively verifies **C6** proofs from at least
   **T+1** parties together with **C7**, which reconstructs the plaintext. Verified on-chain by
   [`BfvDecryptionVerifier`](https://github.com/theinterfold/interfold/blob/main/packages/interfold-contracts/contracts/verifiers/bfv/BfvDecryptionVerifier.sol)
...
[`DkgFoldAttestationVerifier`](https://github.com/theinterfold/interfold/blob/main/packages/interfold-contracts/contracts/verifiers/DkgFoldAttestationVerifier.sol)
...
[`SlashingManager.proposeSlash`](https://github.com/theinterfold/interfold/blob/main/packages/interfold-contracts/contracts/slashing/SlashingManager.sol)
...
3. **Bounded signature window**: every voter signs the same `issuedAt` and `deadline`. The contract
   limits the window to the E3's request-time policy, rejects expired or far-future votes, and
   rejects submissions after the E3's objective reporting deadline. A live zero
   `accusationVoteValidity` pauses new attestation slashes.
...
The faulty-proof bytes are included in the slash proposal and retained on the off-chain
`AccusationQuorumReached` event. The contract hashes those bytes and requires the result to match
the value signed by every voter.
...
`(e3Id, accusationId, dataHash, issuedAt, deadline)` tuple → submitted evidence bytes recovering the
original proof. Both the DKG fault-attribution side and the slashing-evidence side are
cryptographically bound on chain.
...
| **Public key share**                    | The TrBFV public material for that contribution; [**C1**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/pk_generation) proves BFV keygen relations; [**C5**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/pk_aggregation) sums these shares.                                                                                                                     |
| **Secret key share**                    | After P1, a node’s **Shamir** share of the threshold secret (not the same as “contribution”).                                                                                                                                                                                                                                                                                                                               |
| **Threshold public key**                | The BFV key users encrypt to after [**C5**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/pk_aggregation); no single party holds the full matching secret.                                                                                                                                                                                                                                     |
| **Smudging noise**                      | Noise shared across parties so decryption shares do not leak **sk**; **e_sm** in Noir; proved in [**C1**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/pk_generation)–[**C4**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/share_decryption) and used in [**C6**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/share_decryption).      |
| **Smudging noise contribution / share** | Per-node piece of smudging material and its Shamir shares, parallel to the secret-key track ([**C2**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/sk_share_computation_chunk) / [**C3**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/share_encryption) / [**C4**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/share_decryption) smudging track). |
...
[**C2**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/sk_share_computation_chunk)
ships inside the secure preset that
[`config`](https://github.com/theinterfold/interfold/tree/main/circuits/bin/config) checks.
...
[**C0**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/pk) (`dkg/pk`) commits
that key, and later
[**C3**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/share_encryption)
...
[**C2**](https://github.com/theinterfold/interfold/tree/main/circuits/bin/dkg/sk_share_computation_chunk)
is not a single monolithic proof: the repository uses chunked share-computation circuits
`sk_share_computation_chunk` and `esm_share_computation_chunk` to check Shamir-style sharing and
Reed–Solomon parity, groups chunks with `c2_chunk_batch`, and verifies them in the type-bound
terminals `sk_c2_chunk_finalize` / `esm_c2_chunk_finalize` before folding with `c2ab_chunk_fold`.
Intuitively, for each CRT limb and coefficient column, the shares laid out as a vector **y** must
satisfy parity with a fixed matrix **H** so that only low-degree Shamir-style codewords pass (the
exact **PARITY_MATRIX** is preset and checked by `config`):
...
[`user_data_encryption_ct1`](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/user_data_encryption_ct1),
and
[`user_data_encryption`](https://github.com/theinterfold/interfold/tree/main/circuits/bin/threshold/user_data_encryption)—following
...
shares of the threshold secret and smudging noise (from **C4**), and reduction quotients. Each C6
proof also carries the common E3 decryption-domain limbs that are preserved through folding and
checked by the on-chain wrapper. C6 also recomputes the circuit-compatible SAFE commitment from the
decoded `ct0`/`ct1` CRT polynomials; the final decryption proof exposes that commitment for
comparison with the off-chain commitment stored at ciphertext publication—schematically per CRT limb
(see `verify_decryption_share_computation` in `share_decryption.nr`):
...
indices **x_i**. The decryption aggregator requires these Shamir coordinates to be 1-indexed,
strictly increasing, and unique. It then stitches limbs and decodes to the plaintext ring. In short:
...
`q_inverse_mod_t` as in `Configs`). Every decoded coefficient is compared with the claimed plaintext
...
[`recursive_aggregation/`](https://github.com/theinterfold/interfold/tree/main/circuits/bin/recursive_aggregation);
...
this page: the
[`circuits/README.md`](https://github.com/theinterfold/interfold/blob/main/circuits/README.md) index
for package names and **C0–C7** paths;
[DKG & computation](https://github.com/theinterfold/interfold/blob/main/agent/flow-trace/04_DKG_AND_COMPUTATION.md)
