...
| `isLicensed(operator)`         | Whether the bond meets `licenseRequiredBond`                      |
...
> `refreshOperatorStatus` is permissionless. Governance changes to the ticket price, the required
> bond, or the minimum ticket balance invalidate cached status. Call this function to restore the
> correct status for an operator.
