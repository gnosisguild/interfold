...
> **The current Sepolia deployment runs with mock verifiers.** It was deployed with
> `DEPLOY_MOCKS=true` and without `ENABLE_ZK_VERIFICATION`, so no aggregator verifier contracts
> exist on this network and proofs are accepted without being checked. Use it to wire up and operate
> a node, not to test proving correctness.
