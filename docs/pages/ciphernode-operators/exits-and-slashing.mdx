...
struct SlashPolicy {
  ...
  uint256 licensePenalty; // FOLD to confiscate
...
| License penalty | Confiscates FOLD from your bond                  |
...
From the bond-owner wallet or Safe, request deregistration for the operator:
...
```solidity
bondingRegistry.claimExitsFor(operator, maxTicketAmount, maxLicenseAmount);
```
...
| `deregisterOperatorFor(operator)`                | Bond owner | Start the full exit process        |
| `removeTicketBalanceFor(operator, amount)`       | Bond owner | Queue ticket collateral for exit   |
| `unbondLicenseFor(operator, amount)`             | Bond owner | Queue FOLD for exit                |
| `claimExitsFor(operator, maxTicket, maxLicense)` | Bond owner | Claim unlocked assets              |
