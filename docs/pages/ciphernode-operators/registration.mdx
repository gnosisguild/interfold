...
> **Interactive alternative:** the dashboard can do every step on this page in your browser. It
...
```bash
# Register the operator and recompute its activation state
...
interfold --config /path/to/owner.yaml ciphernode deactivate \
  --operator 0xOPERATOR --tickets 50 --license 25
```

`activate` sends the same registration call as `register`. Use it to restore active status after an
eligibility parameter changes on-chain.

`deactivate` requires `--tickets`, `--license`, or both. It calls `removeTicketBalanceFor` for the
ticket amount and `unbondLicenseFor` for the license amount. Both amounts enter the exit queue and
become claimable after the exit delay. The operator stays registered, so use
