...
The two addresses must be different. Ensure the bond-owner wallet has:
...
The owner must be nonzero and different from the operator. It cannot be changed after assignment.
...
receives non-transferable tFOLD. The hot operator key never controls the FOLD or USDC withdrawals.
