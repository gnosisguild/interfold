...
```solidity
usdc.approve(address(ticketToken), amount);
...
```solidity
bondingRegistry.removeTicketBalanceFor(operator, amount);
```
...
The sortition process selects committees deterministically from a shared per-E3 seed:
