...
execution RPC. By default, it waits for one block to build on top of the entropy block; operators do
not configure this depth. Public RPC endpoints use one confirmation automatically, while loopback
development RPC endpoints read the chain head. The first ticket submission derives and stores the
same seed on-chain. No separate randomness transaction is required.
