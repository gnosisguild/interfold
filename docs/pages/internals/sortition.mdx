...
workflow, but it does not rank tickets. A block builder or L2 sequencer cannot calculate alternate
VRF outputs by changing block contents. A provider can delay or withhold a response, but it cannot
replace the frozen request with a second draw. A timeout fails the E3 instead of selecting a
fallback randomness source.
...
The extra nodes act as **standing backups** during committee formation. After finalization, the
runtime uses the address-sorted committee order for party IDs and aggregator failover; ticket-score
order does not assign the aggregator.
...
- If ≥ `threshold_n` tickets were submitted, `SortitionCommitteeFinalized(e3Id, committee, scores)`
  is emitted
...
- Aggregating public-key shares and publishing the committee proof
- Aggregating decryption shares
- Publishing the plaintext output and proof on-chain

If the expected public-key or plaintext result is absent for 10 minutes, or if the active party is
excluded, the next eligible node by `party_id` takes over. The timeout deadline and phase-local
unresponsive set survive restart. Canonical phase progress clears the set, and the final eligible
party remains active after all standby budgets expire.
...
sequenceDiagram
    ...
    NodeB->>Registry: submitTicket(e3Id, ticketId=0)
    ...
    Registry-->>NodeA: SortitionCommitteeFinalized([NodeA, NodeB], [scoreA, scoreB])
    Registry-->>NodeB: SortitionCommitteeFinalized([NodeA, NodeB], [scoreA, scoreB])
...
| **Anti-frontrunning**   | Balances use the timestamp checkpoint at `requestBlock - 1`                    |
